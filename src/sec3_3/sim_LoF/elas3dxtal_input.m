...
pore = 'on';
input_pore_file = 'Defect_Locations_LoF.xlsx';
...
% --- Randomized Orientation Parameters ---
% Yaw
yaw_deg  = 0;
...
% Elongation Factor: Randomly assigns width between 50% and 90% of the length.
elongation_factor = 0.5 + (0.4 * rand(N_pores, 1));

% Prepare cell array for output indices
pore_linear_indices = cell(N_pores,1);

% Generation of LoF Voids
if ~isempty(x_pore)
    fprintf('Inserting %d LoF Voids (Oblate Spheroids)...\n', N_pores);

    for k = 1:N_pores
        % --- A. Geometry: Penny Shape ---
        % 'a' is the major axis (Length along scan track)
        a = D_Feret_max(k)/2; % Major radius

        % 'b' is the minor axis (Width across scan track)
        b = a * elongation_factor(k); % minor radius

        % Protect against division by zero
        if a <= 0 || b <= 0, continue; end

        % 'c' is the thickness (Build direction)
        % Back-calculated from Volume to ensure Mass Conservation
        c = V_pore(k) / ((4/3) * pi * a * b);

        % Safety Check: If data is noisy (e.g., c is NaN or infinite), skip
        if isnan(c) || c <= 0 || isinf(c), continue; end
        ...
        % Create local sub-grid using ndgrid (Correct for 3D matrices)
        [X_sub, Y_sub, Z_sub] = ndgrid(xvec(idx_x), yvec(idx_y), zvec(idx_z));

        % Center coordinates relative to the pore centroid
        Xc = X_sub - x_pore(k);
        Yc = Y_sub - y_pore(k);
        Zc = Z_sub - z_pore(k);
        ...
        % Convert to radians
        phi   = deg2rad(yaw_deg(k));    % Rotation around Z (Yaw)
        theta = deg2rad(tilt_deg(k));   % Rotation around Y (Tilt)

        % First, rotate around Z axis by yaw angle (in-plane spin)
        % Rz (Yaw): In-plane rotation
        Rz = [cos(phi) -sin(phi) 0;
            sin(phi)  cos(phi) 0;
            0         0        1];

        % Then, rotate around Y axis by tilt angle (out-of-plane tip)
        % Ry (Tilt): Out-of-plane tilt
        Ry = [cos(theta) 0 sin(theta);
            0          1 0;
            -sin(theta) 0 cos(theta)];

        % Combined Rotation: First Tilt about Y, then Yaw about Z
        R = Rz * Ry; % 3x3, Total Rotation

        % --- D. Coordinate Transformation ---
        % Flatten meshgrid for affine transformation
        pts = [Xc(:)'; Yc(:)'; Zc(:)'];

        % Apply Inverse Rotation
        pts_local = R' * pts;
        Xr = reshape(pts_local(1,:), size(X_sub));
        Yr = reshape(pts_local(2,:), size(Y_sub));
        Zr = reshape(pts_local(3,:), size(Z_sub));

        % --- E. Ellipsoid Evaluation ---
        % Implicit equation: (x/a)^2 + (y/b)^2 + (z/c)^2 <= 1
        mask_sub = (Xr/a).^2 + (Yr/b).^2 + (Zr/c).^2 <= 1;
    ...
    % Assign the LoF Phase ID
    phase(all_inds) = N_grains + 1;

    % Statistics
    active_pores = sum(~cellfun(@isempty, pore_linear_indices));
    fprintf('LoF Insertion Complete. %d active pores inserted.\n', active_pores);
    nphase = N_grains+1;
else
    nphase = N_grains;
    fprintf('No LoF data found. Skipping insertion.\n');
end
