...
!------------------------------------------------------------------------------ 
      module elas3d_mod
       ...
       ! ---- Microstructure ----
       integer, parameter :: n_grains = 81488    
       ...
       real(dp), parameter :: C11_local = 182.38d9   ! (Pa)
       real(dp), parameter :: C12_local = 143.78d9   ! (Pa)
       real(dp), parameter :: C44_local = 96.50d9    ! (Pa)
       ...
       integer, parameter :: flag_m = 1         ! 0: isotropic matrix, 1: polycrystal       
...
!=========================================================================    
      ...
      ! FILE OPERATIONS      
      open(unit=7, file='output_polycrystal.txt', status='replace', iostat=ios)
...
!-------------------------------------------------------------------------
	  ...
	  open(unit=100, file='cgitr_polycrystal.txt', status='replace', iostat=ios)
      ...
      ! Compute preconditioned inner product
      gg = 0.d0 
      ...
      ! preconditioned conjugate gradient loop
      Lstep = 0
      do ijk = 1, ldemb
        Lstep = Lstep + 1
        ...
        ! Conjugate direction update (preconditioned) (Fletcher-Reeves)
        gamma = gg / gglast
        !$omp parallel do collapse(2) schedule(guided)		
        do i = 1, ndof
		  do m = 1, ns
...
!-------------------------------------------------------------------------  
      ...
      ! Calculate Cofactors for Column 1 of Inverse
	  ...
	  ! Compute Inverse Determinant (1 Division)
      inv_det = 1.0d0 / det	  
      ...
      do k_local = 1, nz
        do j_local = 1, ny
          do i_local = 1, nx
            ...
            ! --- Gather Element Nodal Displacements ---
...
!-------------------------------------------------------------------------
      ...
      ! Set up sample 6x6 stiffness matrix
